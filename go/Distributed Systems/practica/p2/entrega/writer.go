...
func request(ms  *ms.MessageSystem, permission *int, wg *sync.WaitGroup, Log *govec.GoLog){
	...
	for {
		*clockglobal = ra.Request(ms, *myPid, ra.WRITER, *lrd, *clockglobal, permission, state, delay, Log)
...
func main() {
	...
	*lrd = *clockglobal +1										//mi reloj local
	...
	for i := 2; i<=*nProcess +1; i++{					//mensaje a todos
		if(i != *myPid){
			...
			//msg.Send(i, ms.Message{Operation: "RequestWriter " + strconv.Itoa(*lrd), Sender: *myPid})
			vectorclockmessage := Logger.PrepareSend("Sending Message", "RequestWriter " + strconv.Itoa(*lrd), govec.GetDefaultLogOptions())
	...
	*state = 2 //in
	//SC
	msg.Send(1, ms.Message{Operation: ms.START_WRITE, Sender: *myPid})	//luego la creación de un mensaje que es una operation y un sender (mi pid global)
	msg.Send(1, ms.Message{Operation: ms.WRITE, Sender: *myPid})
	msg.Send(1, ms.Message{Operation: ms.WRITE, Sender: *myPid})
	msg.Send(1, ms.Message{Operation: ms.END_WRITE, Sender: *myPid})
